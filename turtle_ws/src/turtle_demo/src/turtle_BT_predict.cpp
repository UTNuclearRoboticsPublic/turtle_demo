...
class TurtleInputNode : public InputDataNode {
    ...
    private:
        std::vector<double> getInputData() override {
            ...
            std::chrono::duration<double> time_diff = std::chrono::system_clock::now() - start_time_;

            // Reset timer when target is spotted (last known pose changes)
            if (!equal_pose(*last_known_pose, temp_pose_)) {
                start_time_ = std::chrono::system_clock::now();
                temp_pose_ = *last_known_pose;
                sighting_count_++;
            }
            ...
            // Input 6: Sighting Count

            return {time_diff.count(), los_dist, chaser_center_distance, energy, sighting_count_};
        }
